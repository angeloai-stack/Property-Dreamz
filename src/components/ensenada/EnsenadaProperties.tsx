"use client";
// "Properties in Ensenada" grid — Figma: "Section / Properties by City Carousel" (nodes 1313:17646-17833).
// Figma repeats one placeholder card 9x; shown here are the real certified listings instead.
...
const ensenadaProperties = properties.filter((p) => p.region === "Ensenada");

export function EnsenadaProperties() {
  const t = useTranslations("ensenada.properties");
      ...
      <div className="mx-auto max-w-7xl px-5 sm:px-8 md:px-12">
        <RevealOnScroll direction="up" className="flex flex-col items-center gap-3 text-center">
          ...
          <p className="font-ewangi text-[14px] text-brand-ink/60">
            <span className="font-bold text-brand-pine">{ensenadaProperties.length}</span> {t("countSuffix")}
          </p>
        </RevealOnScroll>

        <div className="mt-10 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {ensenadaProperties.map((property, i) => {
                ...
                <div className="relative h-75 w-full overflow-hidden">
                  ...
                  <div className="absolute bottom-3 right-3 rounded-[10px] bg-[#024139] px-4 py-2">
                    <span className="font-ewangi text-[15px] font-bold text-white">
                      {t("fromPrefix")} ${Math.round(property.priceUSD / 1000)}K
                    </span>
                  </div>
                </div>
